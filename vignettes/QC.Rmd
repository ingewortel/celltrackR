...
Tracking data can be difficult to analyze because of different errors and artefacts that can occur in the data and cause bias in the analysis. This vignette will explain how the package can be used to detect and correct for some of these errors. The topics will be:

1. Checking track lengths and dealing with short tracks
2. Detecting and correcting tissue drift
3. Detecting and correcting tracking errors and imaging artefacts using angle analyses
...
# 3 Detecting artefacts using angle analyses
...
A plot of angles and distances to the border planes of the imaging volume can help detect artefacts ([Beltman et al (2009)](https://www.ncbi.nlm.nih.gov/pubmed/19834485) ). We can compute these for each step using the functions `distanceToPlane` and `angleToPlane`.
...
Track analysis methods usually assume a constant time interval $\Delta t$ between consecutive images in a time-lapse microscopy dataset. If this assumption does not hold, most of the statistics in this package will be invalid.

In reality, there are mostly at least small fluctuations in the $\Delta t$ between consecutive images. To find the $\Delta t$ of each step, we first extract single steps using the `subtracks()` function (see also the [vignette on analysis methods](./ana-methods.html)):
...
### 4.2 Example: large variation in timesteps

As an example of a dataset where fluctuations in $\Delta t$ are a problem, we will introduce an artificial error by removing some timepoints in a few tracks in the `TCell` data:
...
To see the effect of inconsistent time intervals, let's make a mean squared displacement plot of both the original dataset and the one with gaps:

```{r msd-dt, fig.width=6}
# Compute MSD for TCells
msd <- aggregate( TCells, 
                  squareDisplacement,
                  FUN="mean.se")
msd$gaps <- "no gaps"

# Compute MSD for dataset with gaps
msd.gap <- aggregate( TCells.gap, 
                      squareDisplacement,
                      FUN="mean.se")
msd.gap$gaps <- "gaps"

# Combine data
msd.all <- rbind( msd, msd.gap )

# Filter out time intervals for which at least
# 50% of tracks have a long enough duration
# (otherwise last points in MSD plot will be based
# on very few steps)
time.upbnd <- median( sapply( TCells, nrow) -1 )
msd.all <- msd.all[ msd.all$i <= time.upbnd , ]

# Make the plot
ggplot( msd.all, aes( x = i, y = mean, color = gaps, fill = gaps ) ) +
  geom_path() +
  geom_ribbon( aes(ymin=lower,ymax=upper), alpha = 0.2,
               color = NA) +
  labs( x = "t", y = "displacement^2" ) +
  scale_color_manual( values = c("gaps"="red","no gaps"="gray") ) +
  scale_fill_manual( values = c("gaps"="red","no gaps"="gray") ) +
...
We see that the gaps introduce an overestimation of the MSD.

### 4.3 Correcting variation in timesteps
...
Let's compare the effect of the last two. First, try splitting:

```{r, fig.width=6}
# Repair gaps by splitting
split.gap <- repairGaps( TCells.gap, how = "split" )

# Compute MSD and combine data
msd.split <- aggregate( split.gap, 
                        squareDisplacement,
                        FUN="mean.se")
msd.split$gaps <- "gaps"
msd.split$fix <- "yes (split)"
msd.all$fix <- "no"
d <- rbind( msd.all, msd.split )
d <- d[ d$i <= time.upbnd , ]

# plot
ggplot( d, aes( x = i, y = mean, color = gaps, lty = fix ) ) +
  geom_path() +
  labs( x = "time (#steps)", y = "displacement^2" ) +
  scale_color_manual( values = c("gaps"="red","no gaps"="gray") ) +
  scale_x_continuous( expand=c(0,0) ) +
  scale_y_continuous( expand=c(0,0) ) +
  theme_classic()
...
msd.int <- aggregate( split.gap, 
                      squareDisplacement,
                      FUN="mean.sd")
msd.int$gaps <- "gaps"
msd.int$fix <- "yes (interpolate)"

d <- rbind( msd.all, msd.int )
d <- d[ d$i <= time.upbnd , ]
