...
A qqplot of the two distributions also shows an obvious deviation from a straight line, and a Kolmogorov-Smirnov test confirms that the two distributions are significantly different: 

```{r}
qqplot( step.angles.original, step.angles )
lines( c(0,180),c(0,180), col = "red")
ks.test( step.angles, step.angles.original )
```

For comparison, we do not find this if we compute the angle distributions for two unbiased datasets (compare original T cell data and B cell data):

```{r}
step.angles.B <- sapply( subtracks( BCells, 1 ), angleToDir, dvec = c(1,1,1) )
qqplot( step.angles.original, step.angles.B )
lines( c(0,180),c(0,180), col = "red")
ks.test( step.angles.B, step.angles.original )
```
...
Once again, there is a significant enrichment for small angles:
