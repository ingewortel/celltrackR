...
We now use a similar approach to fit the model proposed by [Beauchemin et al (2007)](https://www.ncbi.nlm.nih.gov/pubmed/17442932), which was designed for T-cell migration in the lymph node. 
...
We should note here that the Beauchemin has three parameters ($v_\text{free}$, $t_\text{free}$, $t_\text{pause}$), but that these are underdefined based on MSD: multiple parameter combinations can give the exact same MSD (see [Beauchemin et al (2007)](https://www.ncbi.nlm.nih.gov/pubmed/17442932), [Textor et al (2013)](https://doi.org/10.1186/1471-2105-14-S6-S10)). This means we cannot fit all three parameters based on the MSD. Here, we'll set $t_\text{pause}$ to the literature value of 0.5 minute, and fit the other parameters $v_\text{free}$ and $t_\text{free}$ based on the MSD to show how this works: 
...
# simulate tracks using the fitted parameters.
...
tau <- timeStep( Txy )
beauchemin.tracks <- simulateTracks( 10,  
