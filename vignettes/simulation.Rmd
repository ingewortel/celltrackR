...
If all is as should be, the distribution of speed and turning angles should now match the real data closely. Check with a qqplot and a KS test:
...
speed.ks <- signif( ks.test( real.speeds, bootstrap.speeds )$p.value )
max.speed <- max( c(real.speeds,bootstrap.speeds))

# Same for turning angles
real.angles <- sapply( subtracks( TCells,2 ), overallAngle )
bootstrap.angles <- sapply( subtracks( bootstrap.tracks,2), overallAngle )
angle.ks <- signif( ks.test( real.angles, bootstrap.angles )$p.value )
max.angle <- max( c(real.angles,bootstrap.angles))


# Plot the result in a qqplot
par(mfrow=c(1,2))
qqplot( real.speeds, bootstrap.speeds,
        main = paste0( "KS p = ", speed.ks ))
lines( c(0,max.speed), c(0,max.speed), col = "red" )
qqplot( real.angles, bootstrap.angles,
        main = paste0( "KS p = ", angle.ks )) 
lines( c(0,max.angle), c(0,max.angle), col = "red" )

```

As it should be, the simulated and real distributions match well to each other.
